...
figure(1);
plot(f(4:end),Bw(4:end),f(4:end),Bs(4:end),f(4:end),B(4:end));
xlim([0 30])
ylim([0 1.2])
legend('"Bw" за хоризонтално направление','"Bs" за вертикално направление', '"B" качество на превозно средство');
xlabel('Честот(Hz)');
ylabel('Тегловни криви за индекс на Шперлинг');
title('Честотни тегловни функции при Шперлинг');
grid;
for a=1:1
    sum=0;sum2=0;
